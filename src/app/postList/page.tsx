        ...
        for (const queryDoc of querySnapshot.docs) {
          ...
          if (postData.driver) {
            ...
            if (driverDoc.exists()) {
              ...
              console.log(
                `Driver document not found for ID: ${postData.driver}`
              );
            }

            const driverInfoDocRef = doc(
              db,
              "Users",
              postData.driver,
              "Profile",
            ...
            if (driverInfoDoc.exists()) {
              ...
              console.log(
                `Driver Info document not found for ID: ${postData.driver}`
              );
            }
          } else {
  ...
  return (
    ...
    >
      <VStack spacing="5" alignItems="left" width="full">
        ...
        {posts.length > 0 ? (
          posts.filter((post) => post.status === "募集中").length > 0 ? ( // "募集中" 状態のポストをフィルタリング
            posts
                ...
                >
                  <HStack>
                    <Box flex="3.5" borderRight="1px solid #ccc" pr="4">
                      ...
                      <HStack justifyContent="space-between">
                        <Box flex="0.75" textAlign="center">
                          <Text>
                            <strong>{post.driverData?.gender}</strong>
                          </Text>
                        </Box>
                        <Box flex="0.75" textAlign="center">
                          <Text>
                            <strong>{post.driverData?.year}</strong>
                          </Text>
                        </Box>
                      </HStack>
                      <Text
                      ...
                      <HStack justifyContent="space-between">
                        <Box flex="0.75" textAlign="center">
                          <Text>
                            <strong>日本語可否:</strong>{" "}
                            {post.driverData?.japaneseProficiency}
                          </Text>
                        </Box>
                        <Box flex="0.75" textAlign="center">
                          <Text>
                            <strong>喫煙:</strong> {post.driverData?.smoking}
                          </Text>
                        </Box>
                      </HStack>
                    </Box>
                    <Box flex="6.5" pl="4">
                      ...
                      >
                        {post.start_time instanceof Timestamp
                      ...
                      >
                        <strong>
                          {post.start_spot} - {post.goal_spot}
                        </strong>
                      </Text>
                      <Text
                        sx={{
                      ...
                      >
                        <strong>車: {post.driverInfo?.carType}</strong>
                      </Text>
                      <Text>
                        <strong>備考:</strong> {post.remarks}
                      </Text>
